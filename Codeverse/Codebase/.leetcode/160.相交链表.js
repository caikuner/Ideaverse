...
/** 1. hashset：时间复杂度 O(m + n) 空间复杂度 O(m)
...
/** 2. 双指针：时间复杂度 O(m + n) 空间复杂度 O(1)
 * @param {ListNode} headA
 * @param {ListNode} headB
 * @return {ListNode}
 */
var getIntersectionNode = function(headA, headB) {
    // 这有点像结合律，当把链表 A、B 合并遍历后，相交节点就是两个指针相遇的地方。
    // 假设从焦点处切割链表各自变成两段，其中交点后面的公共部分为 m，不同的头部分别为 n1和 n2，则 遍历 A: n1 + m + n2;  遍历 B: n2 + m + n1。因此相同速度遍历 n1 + m + n2 步后，必然相遇在交点。

    if (headA === null || headB === null) return null

    let curA = headA, curB = headB
    while (curA !== curB) {
        // 截止：同时遍历到交点 or 同时遍历到链表尾(都没找到交点,null==null)
