...
var widthOfBinaryTree = function (root) {
  ...
  while (que.length) {
    ...
    for (let i = 0; i < curLen; i++) {
      ...
      if (node) {
        if (node.left) que.push([node.left, 2 * index]);
        if (node.right) que.push([node.right, 2 * index + 1]);
      }
    }

    if (que.length) {
      res = Math.max(res, que[que.length - 1][1] - que[0][1] + 1);
    }
  }
  return res;
};
