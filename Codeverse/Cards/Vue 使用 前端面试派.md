...
## 🔥Vue3 和 Vue2 的区别有哪些？说几个主要的 [[https://www.mianshipai.com/docs/second-exam/vue-usage.html#🔥vue3-和-vue2-的区别有哪些-说几个主要的]]
...
## 🔥Vue 组件的通讯方式有哪些？ [[https://www.mianshipai.com/docs/second-exam/vue-usage.html#🔥vue-组件的通讯方式有哪些]]
...
- 获取组件实例对象，调用属性或方法： 💡 Vue 2：通过 `this.$parent` 获取父组件实例，或通过 `this.$children` 获取子组件实例。 💡 Vue 3：通过 `ref` 引用子组件，直接访问其属性和方法。
...
## 🔥Vue 组件的生命周期 [[https://www.mianshipai.com/docs/second-exam/vue-usage.html#🔥vue-组件的生命周期]]
...
1️⃣ **beforeCreate**：组件实例刚被创建，数据观测和事件/监听器设置之前。此时无法访问 `data` 、 `computed` 和 `methods` 等。 2️⃣ **created**：组件实例已创建，数据观测、事件/监听器设置完成，此时可以访问 `data` 、 `computed` 和 `methods` 等，通常用于数据初始化。

- **挂载阶段**

3️⃣ **beforeMount**：在挂载开始之前，模板已编译， `el` 和 `template` 已经确定，但尚未渲染。 4️⃣ **mounted**：组件实例挂载到 DOM 上之后，此时可以访问和操作 DOM。
...
## Vue 组件在哪个生命周期发送 ajax 请求？ [[https://www.mianshipai.com/docs/second-exam/vue-usage.html#vue-组件在哪个生命周期发送-ajax-请求]]
...
## Vue 父子组件生命周期调用顺序 [[https://www.mianshipai.com/docs/second-exam/vue-usage.html#vue-父子组件生命周期调用顺序]]
...
## 🔥v-show 和 v-if 的区别 [[https://www.mianshipai.com/docs/second-exam/vue-usage.html#🔥v-show-和-v-if-的区别]]

参考答案

- **渲染方式：** 💡v-if：条件为 true 时才会渲染元素，条件为 false 时销毁元素。 💡v-show：始终渲染元素，只是通过 CSS 控制 `display 属性` 来显示或隐藏。
- **适用场景：** 💡v-if：适用于条件变化不频繁的场景。 💡v-show：适用于条件变化频繁的场景。

## 为何 v-if 和 v-for 不能一起使用？ [[https://www.mianshipai.com/docs/second-exam/vue-usage.html#为何v-if和v-for不能一起使用]]
...
## computed 和 watch 有什么区别 [[https://www.mianshipai.com/docs/second-exam/vue-usage.html#computed-和-watch-有什么区别]]
...
## 🔥watch 和 watchEffect 的区别 [[https://www.mianshipai.com/docs/second-exam/vue-usage.html#🔥watch-和-watcheffect-的区别]]
...
> Vue 官方 API： [[https://cn.vuejs.org/api/reactivity-core.html#watcheffect]]

## 🔥Vue3 ref 和 reactive 如何选择？ [[https://www.mianshipai.com/docs/second-exam/vue-usage.html#🔥vue3-ref-和-reactive-如何选择]]
...
💡有限的值类型：它只能用于对象类型 (对象、数组和如 Map、Set 这样的集合类型)。它不能持有如 string、number 或 boolean 这样的原始类型。 💡不能替换整个对象：由于 Vue 的响应式跟踪是通过属性访问实现的，因此我们必须始终保持对响应式对象的相同引用。这意味着我们不能轻易地“替换”响应式对象，因为这样的话与第一个引用的响应性连接将丢失：
...
## 什么是动态组件？如何使用它？ [[https://www.mianshipai.com/docs/second-exam/vue-usage.html#什么是动态组件-如何使用它]]

参考答案

动态组件是 Vue 提供的一种机制，允许我们根据条件动态切换渲染的组件，而不需要手动修改模板。 在 Vue 中，我们可以通过 `<component>` 标签的 `:is` 属性指定需要渲染的组件：
...
## 什么是 slot ，有什么应用场景？ [[https://www.mianshipai.com/docs/second-exam/vue-usage.html#什么是-slot-有什么应用场景]]
...
> Vue 官方 API： [[https://cn.vuejs.org/guide/components/slots.html#scoped-slots]]
...
## 🚀Vue 项目可做哪些性能优化？ [[https://www.mianshipai.com/docs/second-exam/vue-usage.html#🚀vue-项目可做哪些性能优化]]
...
## 什么是 nextTick 如何应用它 [[https://www.mianshipai.com/docs/second-exam/vue-usage.html#什么是-nexttick-如何应用它]]
...
## 使用 Vue3 Composable 组合式函数，实现 useCount [[https://www.mianshipai.com/docs/second-exam/vue-usage.html#使用-vue3-composable-组合式函数-实现-usecount]]
...
## 使用 Vue3 Composable 组合式函数，实现 useRequest [[https://www.mianshipai.com/docs/second-exam/vue-usage.html#使用-vue3-composable-组合式函数-实现-userequest]]
...
## 自定义组件如何实现 v-model [[https://www.mianshipai.com/docs/second-exam/vue-usage.html#自定义组件如何实现-v-model]]
...
> 此外，v-model 可以接受自定义参数、添加修饰符，组件也可以绑定多个 v-model ，具体用法请参考 官网文档：[组件 v-model](https://cn.vuejs.org/guide/components/v-model)

## 如何统一监听 Vue 组件报错 [[https://www.mianshipai.com/docs/second-exam/vue-usage.html#如何统一监听-vue-组件报错]]
...
> Vue 官方 API： [[https://cn.vuejs.org/api/composition-api-lifecycle.html#onerrorcaptured]]、[[https://cn.vuejs.org/api/application.html#app-config-errorhandler]]

## Vuex 中 mutation 和 action 有什么区别？ [[https://www.mianshipai.com/docs/second-exam/vue-usage.html#vuex-中-mutation-和-action-有什么区别]]
...
## Vuex 和 Pinia 有什么区别？ [[https://www.mianshipai.com/docs/second-exam/vue-usage.html#vuex-和-pinia-有什么区别]]
...
## Vue-router 导航守卫能用来做什么？ [[https://www.mianshipai.com/docs/second-exam/vue-usage.html#vue-router-导航守卫能用来做什么]]
