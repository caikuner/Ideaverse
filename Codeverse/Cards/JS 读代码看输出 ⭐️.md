...
3. 找函数声明，将函数名作为 GO 对象的 key，值赋为函数体
...
基本类型包括：Undefined、Null、Boolean、Number、String、Symbol 引用类型包括：Object、Array
...
![[JS 读代码  前端面试派-qnBxtptsmL.jpeg]]

- `obj2.y = 2` 能够修改对象的值，`obj1和 obj2` 访问的都是同一个修改后的对象。
- 运行 `obj2 = { y: 20 };` 时，只是将 obj2 指向了新的对象，`obj1` 还是指向原来的对象。

## JS parseInt [[https://www.mianshipai.com/docs/written-exam/JS-reading.html#js-parseint]]
...
查看 MDN 的 [parseInt](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/parseInt) 函数，它的参数是 `(string, radix)`，`string` 是要解析的字符串，`radix` 是 2-36 之间的整数，表示被解析字符串的基数。其中，radix 未指定或者指定 0，基数将会根据字符串的值进行自动推算。
...
作为函数调用时，this 指向全局对象 window。所以 b 为 undefined.
...
this2 window 和 this2 window（严格模式下是 undefined）
```
...
关于 let 声明可以参考 《ECMAScript 6 入门》[[https://es6.ruanyifeng.com/#docs/let]]
...
JavaScript 采用词法作用域 (lexical scoping)，也就是静态作用域。换句话说，函数的作用域在函数定义的时候就决定了。
