...
up:
related:
url: "https://mp.weixin.qq.com/s/qTL3Nvq_hgP6z-oIvaQFsw"
author:
created: 2025-07-17
modified: 2025-07-17
description:
...
Felipe _2025 年 07 月 17 日 08:45_
...
不过接下来就面临另一个问题：客户端需要有某种机制来与 React 的渲染模型进行连接。React 怎么知道客户端的某些状态发生了变化，并需要重新渲染来获取最新的值呢？
...
| Hook           | 说明                               | 场景                      |
| -------------- | ---------------------------------- | ------------------------- |
| useOptimistic  | 支持乐观更新，先显示 UI 后验证数据 | 评论、点赞、输入延迟      |
| useFormStatus  | 表单是否正在提交的状态             | 表单交互体验优化          |
| useActionState | 异步表单动作状态管理               | 与 server action 一起使用 |
...
| 库名           | 架构方式              | 绑定方式               | 特点               |
| -------------- | --------------------- | ---------------------- | ------------------ |
| Redux          | 单一 store + dispatch | Provider + useSelector | 可预测、调试强     |
| Zustand        |                       | 自定义 hook            | 简洁、性能好       |
| Jotai          | 原子化状态            | Provider + useAtom     | 状态粒度细         |
| Recoil         | 数据依赖图            | RecoilRoot + hook      | 自动依赖追踪       |
| TanStack Query | 客户端缓存器          | Provider + useQuery    | 专注异步请求和缓存 |
...
| 需求场景       | 推荐方案                  |
| -------------- | ------------------------- |
| 全局共享状态   | Redux / Zustand           |
| 异步请求管理   | TanStack Query            |
| 服务端表单交互 | React 19 表单新 hooks     |
| 高性能状态更新 | Zustand + 自定义订阅 hook |
| 多依赖状态联动 | Recoil / Jotai            |
| 更强一致性保障 | useSyncExternalStore      |
